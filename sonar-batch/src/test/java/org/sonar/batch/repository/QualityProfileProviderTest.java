...
public class QualityProfileProviderTest {
  ...
  public void testProvide() {
    ...
    when(loader.load("project", null, null)).thenReturn(response);
    ModuleQProfiles qps = qualityProfileProvider.provide(key, loader, projectRepo, props, mode);
    assertResponse(qps);

    verify(loader).load("project", null, null);
  ...
  public void testNonAssociated() {
    ...
    when(loader.loadDefault(null)).thenReturn(response);
    ModuleQProfiles qps = qualityProfileProvider.provide(key, loader, projectRepo, props, mode);
    assertResponse(qps);

    verify(loader).loadDefault(null);
    verifyNoMoreInteractions(loader);
  }
  ...
  public void testProfileProp() {
    when(mode.isNotAssociated()).thenReturn(false);
    when(loader.load("project", "custom", null)).thenReturn(response);
    when(props.property(ModuleQProfiles.SONAR_PROFILE_PROP)).thenReturn("custom");

    ModuleQProfiles qps = qualityProfileProvider.provide(key, loader, projectRepo, props, mode);
    assertResponse(qps);

    verify(loader).load("project", "custom", null);
    verifyNoMoreInteractions(loader);
  }
