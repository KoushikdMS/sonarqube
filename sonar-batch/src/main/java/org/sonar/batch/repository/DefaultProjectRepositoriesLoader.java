...
public class DefaultProjectRepositoriesLoader implements ProjectRepositoriesLoader {
  ...
  private static final String SCANNER_PROJECT_URL = "/scanner/project";
  ...
  private static String getUrl(String projectKey, boolean issuesMode) {
    ...
    builder.append(SCANNER_PROJECT_URL)
  ...
  private static ProjectRepositories processStream(InputStream is, String projectKey) {
    try {
      ...
      for (Map.Entry<String, FileDataByPath> e1 : fileDataByModuleAndPath.entrySet()) {
        for (Map.Entry<String, FileData> e2 : e1.getValue().getFileDataByPath().entrySet()) {
          fileDataTable.put(e1.getKey(), e2.getKey(), e2.getValue());
        }
      }
